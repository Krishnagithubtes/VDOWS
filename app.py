...
def scan():
    ...
    scan_cache['last_result'] = result
    scan_cache['last_url'] = url

    return render_template('result.html', result=result, url=url)

@app.route('/download/<fmt>')
def download(fmt):
    if 'last_result' not in scan_cache:
        return "No scan result available", 400

    result = scan_cache['last_result']
    
    if fmt == 'html':
        url = scan_cache['last_url']
